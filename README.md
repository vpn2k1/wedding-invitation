...
- App admin dùng chung ở `../wedding-admin-app`, chạy ở `/admin` trên port admin
...
Admin dùng chung ở `../wedding-admin-app`:
...
API admin chạy trong `../wedding-admin-app`:
...
Trang `/admin` trong `../wedding-admin-app` và các API `/api/admin/*` được bảo vệ bằng Basic Auth qua `ADMIN_USERNAME` và `ADMIN_PASSWORD` khi chạy production. Ở local development, nếu chưa cấu hình hai biến này thì admin được mở để tiện setup ban đầu. Các Supabase key, service role key và site id đều cấu hình trong `.env.local`, không nhập trên giao diện admin.
...
Sau khi cấu hình, chạy `npm run dev:admin`, vào `/admin/login` ở port admin để đăng nhập rồi quản lý nội dung thiệp.
...
app/
  ...
  api/                       # API settings, album, comments
../wedding-admin-app/
  app/admin/page.tsx         # Trang admin quản lý nội dung và album
  app/api/admin/             # API admin upload/settings
components/
...
- Nâng cấp admin auth sang NextAuth nếu cần nhiều tài khoản.
