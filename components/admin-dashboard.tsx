                ...
                {settings.events.map((event, index) => (
                  <div key={`${event.title}-${index}`} className="rounded-2xl border border-champagne bg-ivory/70 p-4">
                    <div className="mb-4 flex items-center justify-between gap-4">
                      <h2 className="font-serif text-2xl text-wine">Sự kiện {index + 1}</h2>
                      {settings.events.length > 1 && (
                        <button type="button" onClick={() => removeEvent(index)} className="rounded-full border border-wine/20 px-4 py-2 text-sm font-bold text-wine">
                          Xóa
                        </button>
                      )}
                    </div>
                ...
                {settings.qrItems.map((item, index) => (
                  <div key={`${item.ownerName}-${index}`} className="rounded-2xl border border-champagne bg-ivory/70 p-4">
                    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                      <h2 className="font-serif text-2xl text-wine">QR {index + 1}</h2>
                      {settings.qrItems.length > 1 && (
                        <button type="button" onClick={() => removeQrItem(index)} className="rounded-full border border-wine/20 px-4 py-2 text-sm font-bold text-wine">
                          Xóa
                        </button>
                      )}
                    </div>
              ...
              <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
                ...
                <div>
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <h2 className="font-serif text-3xl text-wine">Ảnh đang hiển thị</h2>
                    <button type="button" onClick={loadImages} className="rounded-full border border-wine/20 bg-white/80 px-5 py-3 text-sm font-bold text-wine">
                      Tải lại
                    </button>
                  </div>
                        ...
                        <div className="p-4">
                          ...
                          <div className="mt-4 grid grid-cols-3 gap-2">
                            <button
                              ...
                              className="rounded-full border border-wine/20 px-3 py-2 text-xs font-bold text-wine transition hover:bg-wine hover:text-white disabled:cursor-not-allowed disabled:opacity-40"
                            >
                              Lên
                            </button>
                            <button
